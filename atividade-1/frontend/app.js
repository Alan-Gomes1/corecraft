...
async function refreshNode() {
  ...
  const mpIntEl = document.getElementById("mempoolIntelligence");
  const syncEl = document.getElementById("nodeSyncStatus");
  const errEl = document.getElementById("nodeError");
  const mpIntErrEl = document.getElementById("mempoolIntelligenceError");
  const syncErrEl = document.getElementById("nodeSyncStatusError");

  hideError(errEl);
  hideError(mpIntErrEl);
  hideError(syncErrEl);

  try {
    const [data, mempoolSummary, syncData] = await Promise.all([
      apiGet("/api/node"),
      apiGet("/api/mempool/summary"),
      apiGet("/api/blockchain/lag"),
    ]);
    ...
    setKV(mpIntEl, {
      tx_count: mempoolSummary.tx_count,
      total_vsize: mempoolSummary.total_vsize,
      low: mempoolSummary.min_fee_rate,
      medium: mempoolSummary.avg_fee_rate,
      high: mempoolSummary.max_fee_rate,
    });
    ...
    showError(mpIntErrEl, e.message);
    showError(syncErrEl, e.message);
  }
}
