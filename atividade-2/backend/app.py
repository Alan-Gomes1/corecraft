...
def start_zmq_threads():
    t1 = threading.Thread(
        target=_zmq_subscribe, args=("hashblock", ZMQ_HASHBLOCK), daemon=True
    )
    t2 = threading.Thread(
        target=_zmq_subscribe, args=("hashtx", ZMQ_HASHTX), daemon=True
    )
    t1.start()
    t2.start()
...
def state():
    ...
    return jsonify({
        ...
        "analysis": {
            ...
            "note": (
                "Divergência pode ser latência, reorg, "
                "out-of-order, ou timing do polling."
            )
        },
...
def events_summary():
    with STATE.lock:
        txs = list(STATE.txs)
        blocks_observed = STATE.count_blocks
        txs_observed = STATE.count_txs
        last_event_time = STATE.last_zmq_ts
...
def events_latest():
    ...
    latest_blocks =  [{"hash": ev.value, "ts": ev.ts} for ev in blocks]
    latest_txs = [{"txid": ev.value, "ts": ev.ts} for ev in txs]

    return jsonify({"blocks": latest_blocks, "txs": latest_txs})
